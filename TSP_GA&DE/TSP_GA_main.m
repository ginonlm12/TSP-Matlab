    ...
    %distances = textscan('distance.txt');      
    %N = length(distances);
    POP_SIZE = 100;
    GENERATION = 5000;
    MR = 0.9;
    ...
    for gen = 0 : GENERATION
        % Fitness calculation
        for i = 1: POP_SIZE
            % fitnesses(i) = TSP_GA_fitness_distance(distances, population(i,:));
        ...
        if gen == GENERATION
            for i = 1: POP_SIZE
                if all_fitnesses(:,i) == min(all_fitnesses)
                  ...
                  %filename = 'data.txt'; % Tên file
                  %fileID = fopen(filename, 'w'); % Mở file để ghi
                  %fprintf(fileID, '%d, ', all_population(i,:)'); % Ghi dữ liệu vào file (chuyển vị ma trận)
                  %fclose(fileID); % Đóng file
                end
            end
        end
