...
v2 = zeros(POP_SIZE, N);
v22 = zeros(POP_SIZE, N);

best = [];
best_fit = 1e9; % Objective value

show_iteration_every = 500;
...
% Generate first population through the following 2 methods:
    ...
    % 2. Greedy generation
           % pop1 = TSP_DE_generate_initial_population(N, distance);
           % pop2 = TSP_DE_generate_random_initial_population(POP_SIZE-N, distance);
           % pop(1:N,:) = pop1;
           % pop(N+1:POP_SIZE,:) = pop2;
    tic;
for i = 1:POP_SIZE
...
for gen = 0:GENERATION
    for ii = 1:POP_SIZE
        ...
        v2(ii,:) = pop(k3,:) + F * (pop(k1, :) - pop(k2, :));
        
        v22(ii,:) = TSP_DE_real_to_int(v2(ii,:))';
        
    end
    % Crossover
    offspring = TSP_GA_crossover(v22);
